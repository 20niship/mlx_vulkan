...
// make_euler_devmem_source(nv>80版)の逐語GLSL移植。euler.hppとロジックは同一、質量行列/コレスキー結果だけthread-local配列でなく`L_scratch`(SSBO)にin-place書き込みする点のみ違う(rhs/y/qaccはO(n)なのでthread-localのまま)。
...
inline mkx::fast::Kernel make_euler_devmem_kernel(int nv, int nq, float dt, const std::vector<int>& simple_qa, const std::vector<int>& simple_da, const std::vector<std::pair<int, int>>& free_joints, const std::vector<std::pair<int, int>>& ball_joints, const std::vector<float>& dof_damping_vals) {
  ...
  ss << "uint batch_idx = gl_GlobalInvocationID.x;\n"
     ...
     << "for (uint i = 0u; i < n * n; i++) L_scratch[moff + i] = qM[moff + i];\n";

  for(int i = 0; i < nv; i++) {
    if(dof_damping_vals[static_cast<size_t>(i)] != 0.0f) {
      ss << "L_scratch[moff + " << i * n + i << "] += " << dt_str << "f * " << euler_fmt_float(dof_damping_vals[static_cast<size_t>(i)]) << "f;\n";
    }
  }
  for(int i = 0; i < n; i++) ss << "L_scratch[moff + " << i * n + i << "] += 1e-6f;\n";
  ...
  ss << "float rhs[" << n << "];\n"
     ...
     << "for (uint j = 0u; j < n; j++) {\n"
     << "  float s = 0.0f;\n"
     << "  for (uint k = 0u; k < j; k++) { float v = L_scratch[moff + j*n+k]; s += v*v; }\n"
     << "  float diagv = L_scratch[moff + j*n+j] - s;\n"
     << "  diagv = sqrt(max(diagv, 1e-6f));\n"
     << "  L_scratch[moff + j*n+j] = diagv;\n"
     << "  for (uint i = j+1u; i < n; i++) {\n"
     << "    float s2 = 0.0f;\n"
     << "    for (uint k = 0u; k < j; k++) s2 += L_scratch[moff + i*n+k] * L_scratch[moff + j*n+k];\n"
     << "    L_scratch[moff + i*n+j] = (L_scratch[moff + i*n+j] - s2) / diagv;\n"
     << "  }\n"
     << "}\n"
     ...
     << "for (uint i = 0u; i < n; i++) {\n"
     << "  float s = 0.0f;\n"
     << "  for (uint k = 0u; k < i; k++) s += L_scratch[moff + i*n+k] * y[k];\n"
     ...
     << "for (int i = " << (n - 1) << "; i >= 0; i--) {\n"
     << "  float s = 0.0f;\n"
     << "  for (uint k = uint(i)+1u; k < n; k++) s += L_scratch[moff + k*n+uint(i)] * qacc[k];\n"
  ...
  return mkx::fast::compute_kernel("mjmlx_euler_devmem", {"qM", "qfrc_smooth", "qfrc_constraint", "qvel_in", "qpos_in"}, {"qpos_out", "qvel_out", "qacc_out", "L_scratch"}, ss.str(), "");
}
